...
export async function updateIndustry(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        ...
        `;
        const result = await client.query(query, [industryName, location, industryType, industryEmail, industryId]);
        
        if (result.rowCount === 0){
            return next(new ApiError(404, "Not Found", "Industry not found"));
        }
        return res.status(200).json(new ApiResponse(200, result.rows[0], "Industry updated successfully"));
    } catch (err: any) {
        if (err.code === DbErrorCodes.UNIQUE_VIOLATION) return next(new ApiError(409, "Conflict", "Industry name or email already exists"));
