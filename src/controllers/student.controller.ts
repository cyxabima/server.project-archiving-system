...
export async function createStudent(req: Request, res: Response, next: NextFunction) {
    ...
    if (!seatNo || !stdName || !stdEmail || !batch) {
        return next(new ApiError(422, "Unprocessable Entity", "Seat No, Name, Email, and Batch are required"));
    }
...
export async function updateStudent(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        ...
        return res.status(200).json(new ApiResponse(200, result.rows[0], "Student updated successfully"));
    } catch (err: unknown) {
        const error = err as DatabaseError;
