...
export async function getProjectById(req, res, next) {
    ...
    try {
        ...
        const projectRes = await client.query(`SELECT p.project_id, p.project_title, p.abstract, p.academic_year, p.dept_abbreviation, g.group_id
       FROM projects p
       ...
       WHERE p.project_id = $1`, [projectId]);
        ...
        // 2. Fetch Domains
        const domainRes = await client.query(`SELECT domain_id FROM project_domain WHERE project_id = $1`, [projectId]);
        // 3. Fetch Faculty Supervisors
        const facultyRes = await client.query(`SELECT faculty_id, supervisory_role, remark
       FROM project_faculty WHERE project_id = $1`, [projectId]);
        // 4. Fetch Industries & Externals (Combined)
        const industryRes = await client.query(`SELECT i.industry_name, pi.association_type, pe.ext_email
       ...
       WHERE pi.project_id = $1`, [projectId]);
        // 5. Fetch Grant Details
        const grantRes = await client.query(`SELECT g.grant_name, g.grant_amount, g.recieved_date, i.industry_name
       FROM grants g
       JOIN industry i ON g.industry_id = i.industry_id
       WHERE g.project_id = $1`, [projectId]);
        // 6. Fetch Resources (Files)
        const resourceRes = await client.query(`SELECT resource_name, resource_path
       FROM resources WHERE project_id = $1`, [projectId]);
        ...
        const standardIndustries = industryRes.rows.filter((ind) => ind.association_type !== "Sponsored" || ind.industry_name !== grantSponsorName);
    ...
    try {
        ...
        if (req.query.domainId) {
            ...
            if (typeof req.query.domainId === "string") {
                ...
                domainIdsArray = req.query.domainId;
            }
...
export async function createProject(req, res, next) {
    ...
    const { projectTitle, abstract, academicYear, groupId, grantIndustryName, grantName, grantAmount, recievedDate } = req.body;
    let domainIds = [];
    let facultySupervisors = [];
    let industries = [];
    try {
        ...
        return next(new ApiError(400, "Bad Request", "Invalid JSON format for domains or supervisors or industries."));
    }
    if (!projectTitle ||
        !abstract ||
        !academicYear ||
    ...
    try {
        ...
        const checkGroupRes = await client.query(`SELECT project_id FROM groups WHERE group_id = $1`, [groupId]);
        ...
        const deptRes = await client.query(`SELECT dept_abbreviation FROM domains WHERE domain_id = $1`, [primaryDomainId]);
        if (deptRes.rows.length === 0) {
            throw new Error("DOMAIN_NOT_FOUND");
        }
        const deptAbbr = deptRes.rows[0].dept_abbreviation;
        const projectRes = await client.query(`INSERT INTO projects (project_title, abstract, academic_year, dept_abbreviation)
       VALUES ($1, $2, $3, $4) RETURNING project_id;`, [projectTitle, abstract, academicYear, deptAbbr]);
        const projectId = projectRes.rows[0].project_id;
        // Link the Group
        const groupRes = await client.query(`UPDATE groups SET project_id = $1 WHERE group_id = $2 RETURNING group_id;`, [projectId, groupId]);
        ...
        for (const faculty of facultySupervisors) {
            await client.query(`INSERT INTO project_faculty (project_id, faculty_id, supervisory_role, remark)
         VALUES ($1, $2, $3, $4);`, [projectId, faculty.userId, faculty.role, faculty.remark || null]);
        }
        ...
        for (const ind of industries) {
            ...
            if (indRes.rows.length === 0)
                throw new Error(`INDUSTRY_NOT_FOUND:${ind.industryName}`);
            const industryId = indRes.rows[0].industry_id;
            // Link project to industry
            await client.query(`INSERT INTO project_industry (project_id, industry_id, association_type) VALUES ($1, $2, $3);`, [projectId, industryId, ind.associationType || "Partner"]);
            linkedIndustryIds.add(industryId);
            // Link External Supervisor if provided
            if (ind.extEmail) {
                const extRes = await client.query(`SELECT ext_email FROM external_superv WHERE ext_email = $1`, [ind.extEmail]);
                if (extRes.rows.length === 0)
                    throw new Error(`EXTERNAL_NOT_FOUND:${ind.extEmail}`);
        ...
        if (grantIndustryName && grantName) {
            ...
            if (grantIndRes.rows.length === 0)
                throw new Error(`INDUSTRY_NOT_FOUND:${grantIndustryName}`);
            ...
            if (!linkedIndustryIds.has(grantIndustryId)) {
                await client.query(`INSERT INTO project_industry (project_id, industry_id, association_type) VALUES ($1, $2, $3);`, [projectId, grantIndustryId, "Sponsored"]);
            }
            // Insert Grant Record
            await client.query(`INSERT INTO grants (project_id, grant_name, recieved_date, grant_amount, industry_id)
         VALUES ($1, $2, $3, $4, $5);`, [projectId, grantName, recievedDate || null, grantAmount || null, grantIndustryId]);
        }
        ...
        return res
            .status(201)
            .json(new ApiResponse(201, finalPayload, "Project completely registered and files uploaded successfully."));
    }
    catch (error) {
        ...
        if (error.message === "SUPABASE_UPLOAD_FAILED") {
            return next(new ApiError(502, "Bad Gateway", "Failed to upload files to cloud storage. Database changes rolled back."));
        }
        const dbError = error;
        ...
        if (dbError.code === DbErrorCodes.FOREIGN_KEY_VIOLATION)
            return next(new ApiError(409, "Conflict", "One of the provided Domain IDs or Faculty IDs is invalid."));
...
export async function updateProject(req, res, next) {
    ...
    let industries = [];
    try {
        if (req.body.domainIds)
            domainIds = JSON.parse(req.body.domainIds);
        if (req.body.facultySupervisors)
            facultySupervisors = JSON.parse(req.body.facultySupervisors);
    ...
    try {
        ...
        if (currentGroupId !== groupId) {
            ...
            if (checkNewGroup.rows.length === 0)
                throw new Error("GROUP_NOT_FOUND");
        ...
        if (domainIds.length > 0) {
            ...
            if (deptRes.rows.length > 0)
                deptAbbr = deptRes.rows[0].dept_abbreviation;
        }
        await client.query(`UPDATE projects SET project_title = $1, abstract = $2, academic_year = $3, dept_abbreviation = COALESCE($4, dept_abbreviation) WHERE project_id = $5`, [projectTitle, abstract, academicYear, deptAbbr, projectId]);
        ...
        for (const faculty of facultySupervisors) {
            await client.query(`INSERT INTO project_faculty (project_id, faculty_id, supervisory_role, remark) VALUES ($1, $2, $3, $4);`, [projectId, faculty.userId, faculty.role, faculty.remark || null]);
        }
        ...
        if (grantIndustryName && grantName) {
            ...
            await client.query(`INSERT INTO grants (project_id, grant_name, recieved_date, grant_amount, industry_id) VALUES ($1, $2, $3, $4, $5);`, [projectId, grantName, recievedDate || null, grantAmount || null, grantIndustryId]);
        }
        const safeTitle = projectTitle.replace(/[^a-zA-Z0-9]/g, "_");
        const basePath = `projects/${academicYear}/${projectId}_${safeTitle}`;
        if (reportFile) {
        ...
        if (error.message === "PROJECT_NOT_FOUND")
            return next(new ApiError(404, "Not Found", "Project not found."));
        if (error.message === "GROUP_ALREADY_ASSIGNED")
            return next(new ApiError(409, "Conflict", "The selected group is already assigned to another project."));
        if (error.message === "GROUP_NOT_FOUND")
            return next(new ApiError(404, "Not Found", "Provided Group ID does not exist."));
        if (error.message.startsWith("INDUSTRY_NOT_FOUND"))
            return next(new ApiError(404, "Not Found", `Industry '${error.message.split(":")[1]}' does not exist.`));
...
export async function archiveProject(req, res, next) {
    ...
    try {
        const result = await pool.query(`UPDATE projects SET is_deleted = true WHERE project_id = $1 AND is_deleted = false RETURNING project_id, project_title`, [projectId]);
        if (result.rowCount === 0) {
            return next(new ApiError(404, "Not Found", "Project not found or already archived."));
        }
        return res
            .status(200)
            .json(new ApiResponse(200, result.rows[0], `Project '${result.rows[0].project_title}' archived successfully.`));
    }
    catch (error) {
...
export async function restoreProject(req, res, next) {
    ...
    try {
       ...
       RETURNING project_id, project_title`, [projectId]);
        if (result.rowCount === 0) {
            return next(new ApiError(404, "Not Found", "Project not found or is already active."));
        }
        return res
            .status(200)
            .json(new ApiResponse(200, result.rows[0], `Project '${result.rows[0].project_title}' restored successfully.`));
    }
    catch (error) {
