...
export async function createExternal(req, res, next) {
    ...
    if ([extEmail, extName, industryName].some((field) => !field)) {
        return next(new ApiError(422, "Unprocessable Entity", "Email, Name, and Industry Name are required"));
    }
    try {
...
export async function updateExternal(req, res, next) {
    if (!req.body || Object.keys(req.body).length === 0) {
        return next(new ApiError(422, "Unprocessable Entity", "Body is missing"));
    }
...
export async function getExternals(req, res, next) {
...
export async function deleteExternal(req, res, next) {
    const { extEmail } = req.params;
    try {
        const query = `
    ...
    `;
        ...
        if (err.code === DbErrorCodes.FOREIGN_KEY_VIOLATION) {
            return next(new ApiError(409, "Conflict", "Cannot delete external supervisor because they are currently assigned to active projects or evaluations."));
        }
