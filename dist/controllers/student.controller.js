...
export async function createStudent(req, res, next) {
    ...
    if (!seatNo || !stdName || !stdEmail || !batch || !deptAbbr) {
        return next(new ApiError(422, "Unprocessable Entity", "Seat No, Name, Email, Batch, and Department are required"));
    }
    try {
        const query = `
    ...
    `;
        ...
        if (error.code === DbErrorCodes.UNIQUE_VIOLATION) {
            return next(new ApiError(409, "Conflict", "A student with this Seat Number or Email already exists"));
        }
...
export async function deleteStudent(req, res, next) {
    ...
    `;
        ...
        if (err.code === DbErrorCodes.FOREIGN_KEY_VIOLATION) {
            return next(new ApiError(409, "Conflict", "Cannot delete student because they are currently assigned to a group or project."));
        }
