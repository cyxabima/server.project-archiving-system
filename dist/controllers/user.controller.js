...
export async function addFaculty(req, res, next) {
    if (!req.body || Object.keys(req.body).length === 0) {
        return next(new ApiError(422, "Unprocessable Entity", "Body is missing"));
    }
    const { userName, userEmail, userContactNo, password, deptAbbreviation, designation, areaOfResearch } = req.body;
...
export async function getSupervisingFaculty(req, res, next) {
    try {
        let queryText = `
        SELECT DISTINCT 
            u.user_id, 
    ...
    try {
        ...
        if (role === "admin" && updates.adminLvl !== undefined) {
            ...
            if (facUpdates.length > 0) {
                facVals.push(id);
                await client.query(`UPDATE faculty SET ${facUpdates.join(", ")} WHERE user_id = $${i}`, facVals);
            }
        }
