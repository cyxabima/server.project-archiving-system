...
export async function createGrant(req, res, next) {
    ...
    if ([projectId, grantName, recievedDate, industryName].some((field) => field === undefined)) {
        return next(new ApiError(422, "Unprocessable Entity", "Project ID, Grant Name, Received Date, and Industry Name are required"));
    }
    try {
    ...
    `;
        ...
        if (error.code === DbErrorCodes.UNIQUE_VIOLATION) {
            return next(new ApiError(409, "Conflict", "This grant name already exists for this specific project"));
        }
...
export async function updateGrant(req, res, next) {
    if (!req.body || Object.keys(req.body).length === 0) {
        return next(new ApiError(422, "Unprocessable Entity", "Body is missing"));
    }
...
// GET /api/v1/grants
export async function getGrants(req, res, next) {
